...
app.post("/api/admin/users/:id/trial7", requireAdmin, async (req, res) => {
  try {
    const id = String(req.params.id);
    ...
    const now = new Date();
    const paidUntil = addDays(now, 7);
    const payload = { status: "active", plan_name: "Teste 7 dias", paid_until: paidUntil.toISOString(), grace_until: paidUntil.toISOString(), updated_at: now.toISOString() };
    const license = await getLicense(id);
    if (license) {
    ...
    res.json({ success: true, paid_until: paidUntil.toISOString() });
  } catch (error) {
    console.error("trial7", error);
    res.status(500).json({ success: false, error: "server_error", details: error.message });
  }
});
