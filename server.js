...
<title>Biply Flow - Criar conta</title>
<style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#07111f;color:#fff}.wrap{width:min(560px,92%);margin:45px auto}.card{background:#10243b;border:1px solid #24425f;border-radius:18px;padding:22px}h1{margin:0 0 7px}p{color:#aec0d3;line-height:1.45}input,button{width:100%;padding:13px;border-radius:10px;margin:7px 0;font-size:16px}input{background:#081725;color:white;border:1px solid #31536f}.passwordWrap{position:relative}.passwordWrap input{padding-right:52px}.eye{position:absolute;right:8px;top:12px;width:42px;height:42px;margin:0;background:transparent;color:#d7e4f2;font-size:20px;border:0;cursor:pointer}button{border:0;background:#35e59a;color:#04101a;font-weight:800;cursor:pointer;touch-action:manipulation}.status{margin-top:15px;padding:13px;border-radius:10px;background:#081725;white-space:pre-wrap}.ok{color:#8dffca}.bad{color:#ff9aa5}.textLink{color:#8dffca;text-decoration:underline;font-weight:700}.helper{text-align:center;font-size:13px;margin:10px 0 0}</style>
</head><body><div class="wrap"><div class="card"><h1>Biply Flow</h1><p>Crie sua conta para usar o Biply Flow.</p>
<input id="email" type="email" placeholder="E-mail" autocomplete="email">
<div class="passwordWrap"><input id="password" type="password" placeholder="Senha" autocomplete="new-password"><button id="eyePassword" class="eye" type="button" aria-label="Mostrar ou ocultar senha">&#128065;</button></div>
<div class="passwordWrap"><input id="confirmPassword" type="password" placeholder="Confirmar senha" autocomplete="new-password"><button id="eyeConfirm" class="eye" type="button" aria-label="Mostrar ou ocultar confirmação da senha">&#128065;</button></div>
<button id="signup" type="button">Criar minha conta</button>
<p class="helper">Já possui conta? <a class="textLink" href="/cliente">Entrar</a></p>
<div id="status" class="status">Preencha os dados para criar sua conta.</div></div></div>
<script>
const statusEl=document.getElementById("status");function setStatus(text,cls=""){statusEl.className="status "+cls;statusEl.textContent=text}function toggleEye(inputId){const p=document.getElementById(inputId);p.type=p.type==="password"?"text":"password"}document.getElementById("eyePassword").onclick=()=>toggleEye("password");document.getElementById("eyeConfirm").onclick=()=>toggleEye("confirmPassword");
document.getElementById("signup").onclick=async()=>{const btn=document.getElementById("signup"),email=document.getElementById("email").value.trim(),password=document.getElementById("password").value,confirmPassword=document.getElementById("confirmPassword").value;if(!email||!password||!confirmPassword){setStatus("Preencha e-mail, senha e confirmação da senha.","bad");return}if(password.length<6){setStatus("A senha precisa ter pelo menos 6 caracteres.","bad");return}if(password!==confirmPassword){setStatus("As senhas não conferem.","bad");return}btn.disabled=true;btn.textContent="Criando conta...";setStatus("Criando sua conta...");try{const r=await fetch("/api/signup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password})});const j=await r.json().catch(()=>({}));if(!r.ok){if(j.error==="email_already_registered")setStatus("Este e-mail já possui cadastro. Use a opção Entrar abaixo.","bad");else if(j.error==="invalid_email")setStatus("Digite um e-mail válido.","bad");else setStatus("Não foi possível criar a conta: "+(j.details||j.error||"erro"),"bad");return}document.getElementById("password").value="";document.getElementById("confirmPassword").value="";setStatus("Conta criada com sucesso! Agora você já pode fechar esta página e entrar no Biply Flow com este e-mail e senha.","ok")}catch(e){setStatus("Falha de conexão. Tente novamente.","bad")}finally{btn.disabled=false;btn.textContent="Criar minha conta"}};
</script></body></html>`);
});
...
app.get("/cliente", (req, res) => {
  ...
  res.send(`<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Biply Flow - Área do Cliente</title>
<style>*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#07111f;color:#fff}.wrap{width:min(560px,92%);margin:45px auto}.card{background:#10243b;border:1px solid #24425f;border-radius:18px;padding:22px}h1{margin:0 0 7px}p{color:#aec0d3;line-height:1.45}.passwordWrap{position:relative}input,button{width:100%;padding:13px;border-radius:10px;margin:7px 0;font-size:16px}input{background:#081725;color:white;border:1px solid #31536f}.passwordWrap input{padding-right:52px}.eye{position:absolute;right:8px;top:12px;width:42px;height:42px;margin:0;background:transparent;color:#d7e4f2;font-size:20px;border:0;cursor:pointer}button{border:0;background:#35e59a;color:#04101a;font-weight:800;cursor:pointer;touch-action:manipulation}.secondary{background:#173653;color:white}.status{margin-top:15px;padding:13px;border-radius:10px;background:#081725;white-space:pre-wrap}.ok{color:#8dffca}.bad{color:#ff9aa5}.warn{color:#ffe18b}.hidden{display:none}.plans{display:grid;grid-template-columns:1fr;gap:12px;margin-top:16px}.plan{background:#081725;border:1px solid #31536f;border-radius:14px;padding:15px}.plan h3{margin:0 0 6px}.price{font-size:24px;font-weight:900;margin:6px 0}.save{color:#8dffca;font-size:13px}.pay{background:#27d3ff}.textLink{color:#8dffca;text-decoration:underline;font-weight:700}.helper{text-align:center;font-size:13px;margin:10px 0 0}</style>
</head><body><div class="wrap"><div class="card"><h1>Biply Flow</h1><p>Entre e mantenha esta página aberta enquanto estiver usando a extensão.</p>
<div id="loginBox"><input id="email" type="email" placeholder="E-mail" autocomplete="email"><div class="passwordWrap"><input id="password" type="password" placeholder="Senha" autocomplete="current-password"><button id="eye" class="eye" type="button" aria-label="Mostrar ou ocultar senha">&#128065;</button></div><button id="login" type="button">Entrar</button><p class="helper">Ainda não possui conta? <a class="textLink" href="/cadastro">Criar conta</a></p></div>
...
<script>
let accessToken=null,refreshToken=null,timer=null;const statusEl=document.getElementById("status"),loginBox=document.getElementById("loginBox"),onlineBox=document.getElementById("onlineBox"),account=document.getElementById("account"),monthlyPrice=document.getElementById("monthlyPrice"),annualPrice=document.getElementById("annualPrice"),monthlyDays=document.getElementById("monthlyDays"),annualDays=document.getElementById("annualDays");function setStatus(text,cls=""){statusEl.className="status "+cls;statusEl.textContent=text}function brl(c){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(Number(c||0)/100)}async function loadSettings(){try{const r=await fetch("/api/public/settings");const j=await r.json();if(j.success){monthlyPrice.textContent=brl(j.monthly_price_cents);annualPrice.textContent=brl(j.annual_price_cents);monthlyDays.textContent=j.monthly_days+" dias de acesso";annualDays.textContent=j.annual_days+" dias de acesso"}}catch(e){}}
loadSettings();document.getElementById("eye").onclick=()=>{const p=document.getElementById("password");p.type=p.type==="password"?"text":"password"};async function refresh(){if(!refreshToken)return false;const r=await fetch("/api/token/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({refresh_token:refreshToken})});const j=await r.json().catch(()=>({}));if(!r.ok||!j.access_token)return false;accessToken=j.access_token;refreshToken=j.refresh_token||refreshToken;return true}async function heartbeat(retry=true){if(!accessToken)return;let r=await fetch("/api/session/heartbeat",{method:"POST",headers:{Authorization:"Bearer "+accessToken}});if(r.status===401&&retry&&await refresh())return heartbeat(false);const j=await r.json().catch(()=>({}));if(!r.ok||!j.authorized){setStatus("Acesso não autorizado: "+(j.reason||"erro"),"bad");return}const phase=j.phase==="grace"?"TOLERÂNCIA":"ATIVO";setStatus("SITE CONECTADO - "+phase+"\\nPago até: "+new Date(j.paid_until).toLocaleString("pt-BR")+"\\nTolerância até: "+new Date(j.grace_until).toLocaleString("pt-BR")+"\\n\\nMantenha esta página aberta.",j.phase==="grace"?"warn":"ok")}
document.getElementById("login").onclick=async()=>{const btn=document.getElementById("login"),email=document.getElementById("email").value.trim(),password=document.getElementById("password").value;if(!email||!password){setStatus("Preencha e-mail e senha.","bad");return}btn.disabled=true;btn.textContent="Entrando...";setStatus("Entrando...");try{const r=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password})});const j=await r.json().catch(()=>({}));if(!r.ok||!j.access_token){setStatus("E-mail ou senha inválidos.","bad");return}accessToken=j.access_token;refreshToken=j.refresh_token||null;document.getElementById("password").value="";loginBox.classList.add("hidden");onlineBox.classList.remove("hidden");account.textContent=email;await heartbeat();timer=setInterval(heartbeat,30000)}catch(e){setStatus("Falha de conexão. Tente novamente.","bad")}finally{btn.disabled=false;btn.textContent="Entrar"}};async function createCheckout(plan){if(!accessToken){setStatus("Faça login primeiro.","bad");return}setStatus("Criando pagamento InfinitePay...");let r=await fetch("/api/checkout/create",{method:"POST",headers:{Authorization:"Bearer "+accessToken,"Content-Type":"application/json"},body:JSON.stringify({plan})});if(r.status===401&&await refresh())r=await fetch("/api/checkout/create",{method:"POST",headers:{Authorization:"Bearer "+accessToken,"Content-Type":"application/json"},body:JSON.stringify({plan})});const j=await r.json().catch(()=>({}));if(!r.ok||!j.checkout_url){setStatus("Não foi possível abrir o pagamento.","bad");return}location.href=j.checkout_url}document.getElementById("payMonthly").onclick=()=>createCheckout("monthly");document.getElementById("payAnnual").onclick=()=>createCheckout("annual");document.getElementById("logout").onclick=()=>{accessToken=null;refreshToken=null;if(timer)clearInterval(timer);timer=null;onlineBox.classList.add("hidden");loginBox.classList.remove("hidden");setStatus("Sessão encerrada.")};
</script></body></html>`);
});
